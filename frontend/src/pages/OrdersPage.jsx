  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        console.error('Could not load orders:', fetchError);
          ...
          <div style={{ background: 'var(--bg-card)', padding: '2rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)', minWidth: 0 }}>
            ...
            {loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[1, 2, 3].map((item) => (
                  <div key={item} style={{ height: 90, background: '#f0f0f0', borderRadius: 6, animation: 'pulse 1.5s ease-in-out infinite' }} />
                ))}
              </div>
            ) : error ? (
                    ...
                    <div key={order._id} style={{ border: '1px solid #e0e0e0', borderRadius: 6, overflow: 'hidden' }}>
                      <div style={{ background: '#fafafa', padding: '0.75rem 1.25rem', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontWeight: 700, fontSize: '0.875rem' }}>{formatInr(order.totalPrice)}</p>
                          </div>
                          <div>
                            ...
                            <p style={{ fontWeight: 600, fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: 4 }}>
                              <CreditCard size={12} /> {order.paymentMethod === 'razorpay' ? 'Razorpay' : order.paymentMethod || 'N/A'}
                            </p>
                          </div>
                        </div>
                        <div
                      ...
                      <div style={{ padding: '1rem 1.25rem' }}>
                        {order.orderItems?.map((item, index) => (
                          <div key={`${order._id}-${index}`} style={{ display: 'flex', gap: '1rem', alignItems: 'center', paddingBottom: index < order.orderItems.length - 1 ? '0.75rem' : 0, marginBottom: index < order.orderItems.length - 1 ? '0.75rem' : 0, borderBottom: index < order.orderItems.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                            <div style={{ width: 50, height: 50, borderRadius: 4, overflow: 'hidden', background: '#f5f5f5', flexShrink: 0 }}>
                              <img
                                ...
                                onError={(event) => {
                                  ...
                                  event.target.src = `https://placehold.co/50x50/2874f0/fff?text=${encodeURIComponent((item.name || 'DH').slice(0, 2))}`;
                                }}
                              />
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ fontWeight: 600, fontSize: '0.875rem', marginBottom: '0.15rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {item.name}
                              </p>
                            ...
                            {(order.isPaid || order.status === 'paid') && item.fileUrl && (
                              <a
                                href={item.fileUrl}
                                target="_blank"
                                rel="noreferrer"
